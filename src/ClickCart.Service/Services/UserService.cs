...
public class UserService : IUserService
{
    ...
    public async Task<UserForResultDto> UpdateBalanceAsync(UserForUpdateDto dto)
    {
        var check = await UserRepository.SelectByIdAsync(dto.Id);
        if (check == null)
        {
        ...
        var UserForUpdate = new User()
        {
            ...
            PaymentInformation = check.PaymentInformation,
            City = check.City,
            Country = check.Country,
            StreetAddress = check.StreetAddress,
            ZipPostalCode = check.ZipPostalCode,
            UserRole = Domain.Enums.Roles.Customer,
            UpdatedAt = DateTime.UtcNow,
            CreatedAt = check.CreatedAt,
        };
